...
func (su *SelectUser) DeleteUser(userStorage *user.Storage, w http.ResponseWriter, r http.Request) {
	...
	for index, u := range userStorage.Users {
		if u.Id == su.TargetId {
			...
			_, err = w.Write([]byte(u.Name + " удалён. Статус: " + strconv.Itoa(http.StatusOK)))
			if err != nil {
				return
			}
			return
...
func (su SelectUser) AddFriends(userStorage user.Storage, w http.ResponseWriter, r http.Request) {
	...
	_, err = w.Write([]byte("Пользователь " + name1 + " и Пользователь " + name2 + " друзья! Статус: " + strconv.Itoa(http.StatusOK)))
	if err != nil {
		return
	}
}
